...
// Funções puras movidas para fora para não causarem loop infinito no React
...
const Admin = () => {
  ...
  const buscarDados = useCallback(async () => {
    try {
      ...
      // 1. Busca funcionário logado (Pode retornar 404 e está tudo bem)
      ...
      const resPacientes = await fetch(`${API_URL}/pacientes/painel-admin`).catch(() => null);
      if (resPacientes && resPacientes.ok) {
          const json = await resPacientes.json();
          if (Array.isArray(json)) {
              setPacientes(json.map((item: any) => ({
                  ...
                  status: traduzirStatus(item.status || item.stStatusAtendimento, item.dentista || item.nmDentista),
      ...
      // 3. Busca lista de dentistas
      const resDentistas = await fetch(`${API_URL}/dentistas`).catch(() => null);
      if (resDentistas && resDentistas.ok) {
          const jsonDentistas = await resDentistas.json();
          if (Array.isArray(jsonDentistas)) {
              setListaDentistas(jsonDentistas.map((d: any) => ({
      ...
      // Isso garante que o aviso "Sincronizando..." saia da tela
      setLoading(false);
    }
  }, []);
  ...
  const confirmarAgendamento = async () => {
    if (!modalAgenda || !modalAgenda.idAtendimento) {
        alert("Erro: Este paciente não possui um registro de atendimento válido no sistema.");
        return;
    }
    if (!dentistaSelecionado || !dataSelecionada || !horarioSelecionado) {
    ...
    try {
      ...
      const resAtribuir = await fetch(`${API_URL}/pacientes/${modalAgenda.idAtendimento}/atribuir-medico/${dentistaSelecionado}`, { method: "PUT" });
      if (!resAtribuir.ok) throw new Error("Erro ao vincular dentista.");
      
      const resAgendar = await fetch(`${API_URL}/pacientes/${modalAgenda.idAtendimento}/marcar-consulta`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dataHora: `${dataSelecionada} ${horarioSelecionado}` })
      });
      
      if (!resAgendar.ok) throw new Error("Erro ao salvar horário.");
    ...
    <div className="min-h-screen bg-[#fdfdfc]">
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        ...
        <section className="mt-8 flex flex-col gap-4">
          {loading && <p className="text-sm font-bold text-[#f58200]">Carregando painel...</p>}
          {!loading && pacientesFiltrados.length === 0 && <p className="text-sm text-[#7c6f67]">Nenhum paciente encontrado com estes filtros.</p>}
      ...
      {modalAgenda && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-xl rounded-3xl bg-white p-7 shadow-2xl">
            ...
            <button onClick={confirmarAgendamento} disabled={processando} className="mt-8 h-12 w-full rounded-xl bg-[#f58200] font-bold text-white transition hover:bg-[#ff9d33] disabled:opacity-70">
              {processando ? "Salvando..." : "Confirmar Vínculo e Agenda"}
            </button>
          </div>
        </div>
      )}
