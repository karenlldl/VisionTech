...
const buscarDados = useCallback(async () => {
    try {
      ...
      // 1. Executa as requisições
      const [resAdmin, resPacientes, resDentistas] = await Promise.all([
        fetch(`${API_URL}/funcionarios/${idLogado}`).catch(() => ({ ok: false } as Response)),
        fetch(`${API_URL}/pacientes/painel-admin`),
        fetch(`${API_URL}/dentistas`)
      ]);

      // 2. Processa Admin (com verificação explícita de tipo)
      if (resAdmin.ok) {
  ...
  }, [traduzirPrioridade, traduzirStatus]);
...
setProcessando(true);
    try {
      ...
      const resAtribuir = await fetch(`${API_URL}/pacientes/${modalAgenda.idAtendimento}/atribuir-medico/${dentistaSelecionado}`, { 
        method: "PUT" 
      });
