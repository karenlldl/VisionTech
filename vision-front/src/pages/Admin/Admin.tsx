...
const Admin = () => {
  ...
  const pacientesFiltrados = useMemo(() => {
    return pacientes.filter((paciente) => {
      ...
      const matchOrigem =
        filtroOrigem === "Todas" ||
        (filtroOrigem === "Escola" && paciente.origem !== "Externo") ||
        (filtroOrigem === "Externo" && paciente.origem === "Externo");

      const matchPrioridade =
        filtroPrioridade === "Todas" || paciente.prioridade === filtroPrioridade;

      const matchStatus =
        filtroStatus === "Todos" || paciente.status === filtroStatus;

      const matchProjeto =
        filtroProjeto === "Todos" || paciente.projeto === filtroProjeto;
  ...
  const alterarPrioridade = (id: number, prioridade: string) => {
    setPacientes((prev) =>
      prev.map((paciente) =>
        paciente.id === id ? { ...paciente, prioridade } : paciente
      )
    );
  };

  const confirmarAgendamento = () => {
    if (!modalAgenda || !dentistaSelecionado || !dataSelecionada || !horarioSelecionado) return;
      ...
      prev.map((paciente) =>
        paciente.id === modalAgenda.id
          ? {
              ...
              horarioConsulta: horarioSelecionado,
              status: "Aguardando",
            }
          : paciente
      )
  ...
  const statusStyle = {
    "Sem dentista": "bg-gray-100 text-gray-700",
      ...
      <main className="mx-auto max-w-7xl px-6 py-8">
        ...
        <div>
          ...
          <p className="mt-1 text-[#7c6f67]">
            Gerencie pacientes e acompanhe o processo de atendimento.
          </p>
        </div>
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-5">
          <div className="rounded-2xl border border-[#e7dfd8] bg-white p-5 text-center shadow-sm">
            ...
            <p className="mt-1 text-sm text-[#7c6f67]">Total</p>
          </div>
          ...
          <div className="rounded-2xl border border-[#e7dfd8] bg-white p-5 text-center shadow-sm">
            ...
            <p className="mt-1 text-sm text-[#7c6f67]">Sem dentista</p>
          </div>
          ...
          <div className="rounded-2xl border border-[#e7dfd8] bg-white p-5 text-center shadow-sm">
            ...
            <p className="mt-1 text-sm text-[#7c6f67]">Aguardando</p>
          </div>
          ...
          <div className="rounded-2xl border border-[#e7dfd8] bg-white p-5 text-center shadow-sm">
            ...
            <p className="mt-1 text-sm text-[#7c6f67]">Em atendimento</p>
          </div>
          ...
          <div className="rounded-2xl border border-[#e7dfd8] bg-white p-5 text-center shadow-sm">
            ...
            <p className="mt-1 text-sm text-[#7c6f67]">Concluídos</p>
          </div>
        </section>
          ...
          {pacientesFiltrados.map((paciente) => (
            ...
            >
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-full bg-[#f6f1ec] px-3 py-1 text-sm text-[#7c6f67]">
                    {paciente.escola || paciente.origem || "Externo"}
                  </span>
                </div>
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <span
                    className={`rounded-full px-3 py-1 text-sm font-semibold ${
                      statusStyle[
                        (paciente.status || "Sem dentista") as keyof typeof statusStyle
                      ] || statusStyle["Sem dentista"]
                    }`}
                  >
                    {paciente.status || "Sem dentista"}
                  </span>

                  <span
                    className={`rounded-full px-3 py-1 text-sm font-semibold ${
                      prioridadeStyle[
                        (paciente.prioridade || "Média") as keyof typeof prioridadeStyle
                      ] || prioridadeStyle["Média"]
                    }`}
                  >
                  ...
                  {paciente.dataConsulta && (
                    <span className="rounded-full bg-blue-100 px-3 py-1 text-sm text-blue-700">
                      {paciente.dataConsulta} • {paciente.horarioConsulta}
                    </span>
                  )}
              ...
              <div className="flex flex-wrap items-center gap-3">
                {/* PRIORIDADE */}
                <select
                  value={paciente.prioridade || "Média"}
                  onChange={(e) => alterarPrioridade(paciente.id, e.target.value)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-2xl rounded-3xl bg-white p-7 shadow-2xl">
              ...
              <div>
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`rounded-full px-3 py-1 text-sm font-semibold ${
                      statusStyle[
                        (pacienteSelecionado.status || "Sem dentista") as keyof typeof statusStyle
                      ] || statusStyle["Sem dentista"]
                    }`}
                  >
                    {pacienteSelecionado.status || "Sem dentista"}
                  </span>
                </div>
              </div>
            ...
            <div className="mt-6 grid gap-4 text-[#4d4039] md:grid-cols-2">
              <p>
                <strong>Idade:</strong> {pacienteSelecionado.idade}
              </p>

              <p>
                <strong>Origem:</strong> {pacienteSelecionado.escola || pacienteSelecionado.origem || "Externo"}
              </p>

              <p>
                <strong>Dentista:</strong> {pacienteSelecionado.dentista || "Não atribuído"}
              </p>

              <p>
                <strong>Projeto:</strong> {pacienteSelecionado.programa || pacienteSelecionado.projeto || "Não definido"}
              </p>

              <p>
                <strong>Consulta:</strong> {pacienteSelecionado.dataConsulta || "Não agendada"}
              </p>

              <p>
                <strong>Horário:</strong> {pacienteSelecionado.horarioConsulta || "--"}
              </p>
            </div>

            <div className="mt-6">
              <h3 className="font-semibold text-[#2f251f]">Observações</h3>
              <div className="mt-2 rounded-2xl bg-[#f6f1ec] p-4 text-[#5f534c]">
                {pacienteSelecionado.observacao || "Nenhuma observação registrada."}
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="w-full max-w-xl rounded-3xl bg-white p-7 shadow-2xl">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-[#7c6f67]">{modalAgenda.nome}</p>
              </div>

              <button onClick={() => setModalAgenda(null)}>
                <X className="h-5 w-5 text-[#7c6f67]" />
              </button>
            </div>
            ...
            <div className="mt-5">
              ...
              <div className="mt-3 grid grid-cols-3 gap-3">
                {["08:00", "09:00", "10:00", "11:00", "14:00", "15:00"].map((hora) => (
                  <button
                    key={hora}
